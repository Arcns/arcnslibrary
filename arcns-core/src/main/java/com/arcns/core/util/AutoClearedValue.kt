 ...
 * 可解决DataBinding等对象造成Fragment内存泄漏等问题
 */
class AutoClearedValueOrNull<T : Any?>(
    val fragment: Fragment,
    val onCreateCallback: (() -> T?)? = null,
    val onClearCallback: ((value: T?) -> Unit)? = null
) :
    ...
    override fun getValue(thisRef: Fragment, property: KProperty<*>): T? = _value
 ...
 */
fun <T : Any?> Fragment.autoClearedOrNull(
    onCreateCallback: (() -> T?)? = null,
    onClearCallback: ((value: T?) -> Unit)? = null
) =
    AutoClearedValueOrNull(this, onCreateCallback, onClearCallback)
